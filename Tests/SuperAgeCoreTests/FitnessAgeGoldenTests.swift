...
struct GoldenFixture {
    ...
    static func load(fileID: String = #fileID) -> GoldenFixture {
        guard let fixtureURL = Bundle.module.url(
            forResource: "fitness_age_golden",
            withExtension: "json"
        ) else {
            fatalError("\(fileID) fitness_age_golden.json missing from the test bundle resources")
        }
