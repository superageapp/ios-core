...
struct FitnessAgeDomainScorer {
    ...
    private func calculateCardiovascularScore(
        ...
        return FitnessAgeDomainScore(
            score: finalScore,
            percentile: estimatedRelativeStanding(score: finalScore),
    ...
    private func calculateRecoveryScore(
        ...
        } else if allowsSleepFallback, totalWeight > 0 {
            // The fallback preserves continuity when a caller supplies other recovery data
            // but no sleep observation; with no observed recovery metric at all it must not
            // manufacture a recovery domain out of nothing.
    ...
    // A synthetic affine rescale of the domain score, not a measured population
    // percentile; see "Domain relative standing" in Docs/METHODOLOGY.md.
    private func estimatedRelativeStanding(score: Double) -> Double {
