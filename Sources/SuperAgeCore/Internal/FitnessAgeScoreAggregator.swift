...
enum FitnessAgeScoreAggregator {
    ...
    static func confidence(
        ...
        // Observability matches the scoring rule: a supplied numeric 0 is an omitted
    ...
    private static func dataQuality(for metrics: FitnessAgeMetrics) -> Double {
        ...
        if (metrics.stepCount ?? 0) > 0 {
            qualityComponents.append((0.85, 0.23))
        }
        if (metrics.activeEnergy ?? 0) > 0 {
            qualityComponents.append((0.85, 0.17))
        }
        if (metrics.exerciseTime ?? 0) > 0 {
            qualityComponents.append((0.85, 0.17))
        }
        if (metrics.sleepHours ?? 0) > 0 || (metrics.sleepScore ?? 0) > 0 {
            qualityComponents.append((0.85, 0.23))
        }
