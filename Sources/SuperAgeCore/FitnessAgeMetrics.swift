...
public struct FitnessAgeMetrics: Codable, Equatable, Sendable {
    ...
    /// Smoking status; `false` is observed data and maps to the top score for the metric.
...
public extension FitnessAgeMetrics {
    /// Clears the metric fields mapped to the given disabled metric IDs.
    ///
    /// See "Recognized disabled metric IDs" in Docs/METHODOLOGY.md for the exact ID
    /// strings and the fields each one clears. Unrecognized IDs are ignored.
    ...
    /// Returns a copy with the metric fields mapped to the given disabled metric IDs cleared.
